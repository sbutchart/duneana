...
enum PType{kUnknown=0, kMarl, kAr39_Lar, kAr42_Lar, kK42_Lar, kKr85_Lar, kRn222_Lar, kPo218_Lar, kPb214_Lar, kBi214_Lar, kPb210_Lar, kK40_CPA, kAr42_CPA, kU238_CPA, kPo218_CPA, kPb214_CPA, kBi214_CPA, kPb210_CPA, kBi210_CPA, kCo60_APA, kU238_APA, kRn222_PDS, kNeut, kCavNGam, kCavwallGam, kFoamGam, kCryoGam, kRn220_Lar, kRn220_CPA, kTh232_APA};
...
std::map<PType, std::string> PTypeString{
  ...
  {kNeut     ,"Neut"     },
  {kCavNGam  ,"CavNGam"  },
  {kCavwallGam,"CavwallGam"},
  {kFoamGam  ,"FoamGam"},
  {kCryoGam  ,"CryoGam"},
  {kRn220_Lar,"Rn220_Lar"},
  {kRn220_CPA,"Rn220_CPA"},
  {kTh232_APA,"Th232_APA"}
};


class SNAnaUpdate : public art::EDAnalyzer {

public:
  explicit SNAnaUpdate(fhicl::ParameterSet const & p);

  SNAnaUpdate(SNAnaUpdate const &) = delete;
  SNAnaUpdate(SNAnaUpdate &&) = delete;
  SNAnaUpdate & operator = (SNAnaUpdate const &) = delete;
  SNAnaUpdate & operator = (SNAnaUpdate &&) = delete;
...
private:
  ...
  //New & updated in Model 3:
  
  std::string fNeutLabel      ; std::map< int, simb::MCParticle > NeutParts;
  
  std::string fCavNGamLabel   ; std::map< int, simb::MCParticle > CavNGamParts;
  std::string fCavwallGamLabel; std::map< int, simb::MCParticle > CavwallGamParts;
  std::string fFoamGamLabel   ; std::map< int, simb::MCParticle > FoamGamParts;
  std::string fCryoGamLabel   ; std::map< int, simb::MCParticle > CryoGamParts;
  
  std::string fRn220_LarLabel ; std::map< int, simb::MCParticle > Rn220_LarParts;
  std::string fTh232_APALabel ; std::map< int, simb::MCParticle > Th232_APAParts;
  std::string fRn220_CPALabel ; std::map< int, simb::MCParticle > Rn220_CPAParts;
  ...
  int   TotGen_Rn220_Lar;
  int   TotGen_Th232_APA;
  int   TotGen_Rn220_CPA;
...
SNAnaUpdate::SNAnaUpdate(fhicl::ParameterSet const & p):EDAnalyzer(p),
                                            fname("SNAnaUpdate_module")
{
  this->reconfigure(p);
}


void SNAnaUpdate::reconfigure(fhicl::ParameterSet const & p)
{
  ...
  //This is where we read in the labels as written in SNAna.fcl
  ...
  fCavNGamLabel    = p.get<std::string> ("CavernNGammasLabel");
  fCavwallGamLabel = p.get<std::string> ("CavernWallGammasLabel");
  fFoamGamLabel   = p.get<std::string> ("FoamGammasLabel");
  fCryoGamLabel   = p.get<std::string> ("CryostatGammasLabel");
  fRn220_LarLabel = p.get<std::string> ("Rn220Cha_Lar_Label");
  fRn220_CPALabel = p.get<std::string> ("Rn220Cha_CPA_Label");
  fTh232_APALabel = p.get<std::string> ("Th232Cha_APA_Label");
...
void SNAnaUpdate::ResetVariables()
{
  trkIDToPType.clear();
  ...
  Rn222_PDSParts.clear(); NeutParts     .clear(); CavNGamParts  .clear(); CavwallGamParts.clear();
  FoamGamParts  .clear(); CryoGamParts  .clear(); Rn220_LarParts.clear(); Th232_APAParts.clear();
  Rn220_CPAParts.clear();
  ...
  TotGen_Rn222_PDS = TotGen_Neut      = TotGen_CavNGam    = TotGen_CavwallGam = 0;
  TotGen_FoamGam   = TotGen_CryoGam   = TotGen_Rn220_Lar = TotGen_Th232_APA = 0;
  TotGen_Rn220_CPA = 0;
...
void SNAnaUpdate::beginJob()
{
  ...
  fSNAnaTree->Branch("TotGen_CavNGam"  , &TotGen_CavNGam  , "TotGen_CavNGam/I"  );
  fSNAnaTree->Branch("TotGen_CavwallGam", &TotGen_CavwallGam, "TotGen_CavwallGam/I");
  fSNAnaTree->Branch("TotGen_FoamGam"  , &TotGen_FoamGam  , "TotGen_FoamGam/I"  );
  fSNAnaTree->Branch("TotGen_CryoGam"  , &TotGen_CryoGam  , "TotGen_CryoGam/I"  );
  fSNAnaTree->Branch("TotGen_Rn220_Lar", &TotGen_Rn220_Lar, "TotGen_Rn220_Lar/I");
  fSNAnaTree->Branch("TotGen_Th232_APA", &TotGen_Th232_APA, "TotGen_Th232_APA/I");
  fSNAnaTree->Branch("TotGen_Rn220_CPA", &TotGen_Rn220_CPA, "TotGen_Rn220_CPA/I");

}


void SNAnaUpdate::analyze(art::Event const & evt)
{
  ResetVariables();
  ...
  if (MarlTrue) {
    ...
    for(size_t i = 0; i < MarlTrue->size(); i++)
    {
      ...
      try {
        ...
        mf::LogDebug(fname) << "Didnt find SN truth (few things weren't available):\n"
                            << " - MarlTime\n"
                            << " - MarlWeight\n"
                            << " - MarlSample\n";
      }
  ...
  auto CavNGamTrue = evt.getHandle< std::vector<simb::MCTruth> > (fCavNGamLabel);
  if(CavNGamTrue) {
    art::FindManyP<simb::MCParticle> CavNGamAssn(CavNGamTrue,evt,fGEANTLabel);
    FillMyMaps( CavNGamParts, CavNGamAssn, CavNGamTrue );
    TotGen_CavNGam = CavNGamParts.size();
    if(fSaveTruth) FillTruth(CavNGamAssn , CavNGamTrue , kCavNGam );
  }

  auto CavwallGamTrue = evt.getHandle< std::vector<simb::MCTruth> > (fCavwallGamLabel);
  if(CavwallGamTrue) {
    art::FindManyP<simb::MCParticle> CavwallGamAssn(CavwallGamTrue,evt,fGEANTLabel);
    FillMyMaps( CavwallGamParts, CavwallGamAssn, CavwallGamTrue );
    TotGen_CavwallGam = CavwallGamParts.size();
    if(fSaveTruth) FillTruth(CavwallGamAssn , CavwallGamTrue , kCavwallGam );
  }
  
  auto FoamGamTrue = evt.getHandle< std::vector<simb::MCTruth> > (fFoamGamLabel);
  if(FoamGamTrue) {
    art::FindManyP<simb::MCParticle> FoamGamAssn(FoamGamTrue,evt,fGEANTLabel);
    FillMyMaps( FoamGamParts, FoamGamAssn, FoamGamTrue );
    TotGen_FoamGam = FoamGamParts.size();
    if(fSaveTruth) FillTruth(FoamGamAssn , FoamGamTrue , kFoamGam );
  } 
  
  auto CryoGamTrue = evt.getHandle< std::vector<simb::MCTruth> > (fCryoGamLabel);
  if(CryoGamTrue) {
    art::FindManyP<simb::MCParticle> CryoGamAssn(CryoGamTrue,evt,fGEANTLabel);
    FillMyMaps( CryoGamParts, CryoGamAssn, CryoGamTrue );
    TotGen_CryoGam = CryoGamParts.size();
    if(fSaveTruth) FillTruth(CryoGamAssn , CryoGamTrue , kCryoGam );
  } 

  auto Rn220_LarTrue = evt.getHandle< std::vector<simb::MCTruth> > (fRn220_LarLabel);
  if(Rn220_LarTrue) {
    art::FindManyP<simb::MCParticle> Rn220_LarAssn(Rn220_LarTrue,evt,fGEANTLabel);
    FillMyMaps( Rn220_LarParts, Rn220_LarAssn, Rn220_LarTrue );
    TotGen_Rn220_Lar = Rn220_LarParts.size();
    if(fSaveTruth) FillTruth(Rn220_LarAssn , Rn220_LarTrue , kRn220_Lar );
  } 

  auto Th232_APATrue = evt.getHandle< std::vector<simb::MCTruth> > (fTh232_APALabel);
  if(Th232_APATrue) {
    art::FindManyP<simb::MCParticle> Th232_APAAssn(Th232_APATrue,evt,fGEANTLabel);
    FillMyMaps( Th232_APAParts, Th232_APAAssn, Th232_APATrue );
    TotGen_Th232_APA = Th232_APAParts.size();
    if(fSaveTruth) FillTruth(Th232_APAAssn , Th232_APATrue , kTh232_APA );
  } 

  auto Rn220_CPATrue = evt.getHandle< std::vector<simb::MCTruth> > (fRn220_CPALabel);
  if(Rn220_CPATrue) {
    art::FindManyP<simb::MCParticle> Rn220_CPAAssn(Rn220_CPATrue,evt,fGEANTLabel);
    FillMyMaps( Rn220_CPAParts, Rn220_CPAAssn, Rn220_CPATrue );
    TotGen_Rn220_CPA = Rn220_CPAParts.size();
    if(fSaveTruth) FillTruth(Rn220_CPAAssn , Rn220_CPATrue , kRn220_CPA );
  } 
  ...
  for(auto& it: NeutParts)
    allTruthParts.push_back(it.second);
  for(auto& it: CavNGamParts)
    allTruthParts.push_back(it.second);
  for(auto& it: CavwallGamParts)
    allTruthParts.push_back(it.second);
  for(auto& it: FoamGamParts)
    allTruthParts.push_back(it.second);
  for(auto& it: CryoGamParts)
    allTruthParts.push_back(it.second); 
  for(auto& it: Rn220_LarParts)
    allTruthParts.push_back(it.second);    
  for(auto& it: Th232_APAParts)
    allTruthParts.push_back(it.second); 
  for(auto& it: Rn220_CPAParts)
    allTruthParts.push_back(it.second);     
  ...
  std::map<PType, std::map< int, simb::MCParticle >&> PTypeToMap{
    ...
    {kNeut,      NeutParts     },
    {kCavNGam,   CavNGamParts  },
    {kCavwallGam, CavwallGamParts},
    {kFoamGam,   FoamGamParts  },
    {kCryoGam,   CryoGamParts  },
    {kRn220_Lar, Rn220_LarParts},
    {kTh232_APA, Th232_APAParts},
    {kRn220_CPA, Rn220_CPAParts},
  };
  ...
  if (fSaveTPC) {
    ...
    if ( reco_hits && rawDigitsVecHandle ) {
      ...
      std::vector< recob::Hit > ColHits_CavNGam  ;
      std::vector< recob::Hit > ColHits_CavwallGam;
      std::vector< recob::Hit > ColHits_FoamGam  ;
      std::vector< recob::Hit > ColHits_CryoGam  ;
      std::vector< recob::Hit > ColHits_Rn220_Lar;
      std::vector< recob::Hit > ColHits_Th232_APA;
      std::vector< recob::Hit > ColHits_Rn220_CPA;
      ...
      for(int hit = 0; hit < LoopHits; ++hit) {
        ...
        if (ThisHit.View() == 2) {
          ...
          else if (ThisPType == 23) { ColHits_CavNGam  .push_back( ThisHit ); }
          else if (ThisPType == 24) { ColHits_CavwallGam.push_back(ThisHit );}
          else if (ThisPType == 25) { ColHits_FoamGam  .push_back( ThisHit ); }
          else if (ThisPType == 26) { ColHits_CryoGam  .push_back( ThisHit ); }
          else if (ThisPType == 27) { ColHits_Rn220_Lar.push_back( ThisHit ); }
          else if (ThisPType == 28) { ColHits_Th232_APA.push_back( ThisHit ); }
          else if (ThisPType == 29) { ColHits_Rn220_CPA.push_back( ThisHit ); }
      ...
      mf::LogInfo(fname) << "Total of:\n"
                         ...
                         << " - CavNGam  : " << TotGen_CavNGam     << " true parts\t| " << ColHits_CavNGam  .size() << " col plane hits\n"
                         << " - CavwallGam: " << TotGen_CavwallGam << " true parts\t| " << ColHits_CavwallGam.size() << " col plane hits\n"
                         << " - FoamGam  : " << TotGen_FoamGam     << " true parts\t| " << ColHits_FoamGam  .size() << " col plane hits\n"
                         << " - CryoGam  : " << TotGen_CryoGam     << " true parts\t| " << ColHits_CryoGam  .size() << " col plane hits\n"
                         << " - Rn220_Lar: " << TotGen_Rn220_Lar   << " true parts\t| " << ColHits_Rn220_Lar.size() << " col plane hits\n"
                         << " - Th232_APA: " << TotGen_Th232_APA   << " true parts\t| " << ColHits_Th232_APA.size() << " col plane hits\n"
                         << " - Rn220_CPA: " << TotGen_Rn220_CPA   << " true parts\t| " << ColHits_Rn220_CPA.size() << " col plane hits\n";
  ...
  if (fSavePDS) {
    ...
    if (OpHitHandle) {
      ...
      mf::LogInfo(fname) << "Total of :\n"
                         ...
                         << " - CavNGam  : " << TotGen_CavNGam   << " true parts\t| " << map_of_ophit[kCavNGam]   .size() << " opt hits\n"
                         << " - CavwallGam: " << TotGen_CavwallGam << " true parts\t| " << map_of_ophit[kCavwallGam].size() << " opt hits\n"
                         << " - FoamGam  : " << TotGen_FoamGam   << " true parts\t| " << map_of_ophit[kFoamGam]   .size() << " opt hits\n"
                         << " - CryoGam  : " << TotGen_CryoGam   << " true parts\t| " << map_of_ophit[kCryoGam]   .size() << " opt hits\n"
                         << " - Rn220_Lar: " << TotGen_Rn220_Lar << " true parts\t| " << map_of_ophit[kRn220_Lar] .size() << " opt hits\n"
                         << " - Th232_APA: " << TotGen_Th232_APA << " true parts\t| " << map_of_ophit[kTh232_APA] .size() << " opt hits\n"
                         << " - Rn220_CPA: " << TotGen_Rn220_CPA << " true parts\t| " << map_of_ophit[kRn220_CPA] .size() << " opt hits\n";
...
void SNAnaUpdate::FillTruth(const art::FindManyP<simb::MCParticle> Assn,
                      const art::Handle<std::vector<simb::MCTruth>>& Hand,
                      const PType type) {
...
void SNAnaUpdate::FillMyMaps(std::map< int, simb::MCParticle> &MyMap,
                       art::FindManyP<simb::MCParticle> Assn,
                       art::Handle< std::vector<simb::MCTruth> > Hand,
...
PType SNAnaUpdate::WhichParType( int TrID )
{
...
bool SNAnaUpdate::InMyMap( int TrID, std::map< int, simb::MCParticle> ParMap )
{
...
//......................................................
void SNAnaUpdate::SaveIDEs(art::Event const & evt)
{
...
void SNAnaUpdate::SaveNeighbourADC(int channel,
                             art::Handle< std::vector<raw::RawDigit> >rawDigitsVecHandle,
                             std::set<int>badChannels,
