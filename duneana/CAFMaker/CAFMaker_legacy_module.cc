////////////////////////////////////////////////////////////////////////
//
// \file CAFMaker_legacy_module.cc
//
...
#ifndef CAFMaker_legacy_H
#define CAFMaker_legacy_H
...
namespace caf {
  ...
  class CAFMaker_legacy : public art::EDAnalyzer {

    public:

      explicit CAFMaker_legacy(fhicl::ParameterSet const& pset);
      virtual ~CAFMaker_legacy();
  ...
  }; // class CAFMaker_legacy


  //------------------------------------------------------------------------------
  CAFMaker_legacy::CAFMaker_legacy(fhicl::ParameterSet const& pset)
    : EDAnalyzer(pset)
  {
    ...
    // Get DUNErw stuff from its fhicl, which should be included on the CAFMaker_legacy config file
  ...
  //------------------------------------------------------------------------------
  caf::CAFMaker_legacy::~CAFMaker_legacy()
  {
  }

  //------------------------------------------------------------------------------
  void CAFMaker_legacy::beginJob()
  {
    if(fOutFile){
      fOutFile->cd();
  ...
  //------------------------------------------------------------------------------
  void CAFMaker_legacy::AddGlobalTreeToFile(TFile* f, SRGlobal& global)
  {
    f->cd();
  ...
  void CAFMaker_legacy::FillTruthInfo(caf::StandardRecord& sr,
                               std::vector<simb::MCTruth> const& truth,
                               std::vector<simb::MCFlux> const& flux,
                               art::Event const& evt) const
  {
    for(size_t i=0; i<truth.size(); i++){
      ...
      if(i>1){
        mf::LogWarning("CAFMaker_legacy") << "Skipping MC truth index " << i;
        continue;
      }
  ...
  //------------------------------------------------------------------------------
  void CAFMaker_legacy::beginSubRun(const art::SubRun& sr)
  {
  ...
  //------------------------------------------------------------------------------
  void CAFMaker_legacy::analyze(art::Event const & evt)
  {
    ...
    if ( !mct ) {
      mf::LogWarning("CAFMaker_legacy") << "No MCTruth.";
        }
    else if ( !mct->empty() ) {
  ...
  //------------------------------------------------------------------------------
  void CAFMaker_legacy::endSubRun(const art::SubRun& sr){
  }

  void CAFMaker_legacy::endJob()
  {
    fMetaTree->Fill();
  ...
  DEFINE_ART_MODULE(CAFMaker_legacy)

} // namespace caf

#endif // CAFMaker_legacy_H
